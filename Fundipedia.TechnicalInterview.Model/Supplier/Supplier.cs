using Fundipedia.TechnicalInterview.Helpers;
using System;
...
public class Supplier : IValidatableObject
{
    ...
    public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        {
            if (!ValidationHelper.IsValidEmail(email.EmailAddress))
            {
                validationResults.Add(new ValidationResult("Email address has to be a valid format.", new[] { nameof(Emails) }));
                break;
            }
        }
        ...
        {
            if (!ValidationHelper.IsValidPhoneNumber(phone.PhoneNumber))
            {
                validationResults.Add(new ValidationResult("Phone number has to be numeric and with a max length of 10.", new[] { nameof(Phones) }));
                break;
            }
        }
